...
int main(int argc, char* argv[]) {
  ...
  try {
    ...
    std::string system_root_path_ = argv[2];

    Config config(system_root_path_);

    logger->info("Запуск определения версии Windows");
    logger->debug("Используется файл реестра: {}", registry_file);

    // 3. Создание детектора ОС
    OSDetection detector(std::move(parser), (std::move(config)), registry_file);
    ...
    std::cout << "Операционная система: " << os_info.full_os_name << "\n";
    std::cout << "Название продукции: " << os_info.product_name << "\n";
    ...
    if (!os_info.release.empty()) {
      std::cout << "Идентификатор выпуска: " << os_info.release << "\n";
    }
